...
const SectionTitle = ({title  , subTitle , modeBlue}) => {
  return (
      <div className={`${modeBlue ? 'gap-2 md:gap-5'  :'gap-y-6 md:gap-y-10' }   flex flex-col items-center  `}>
        <h2 className={`text-3xl md:text-5xl font-forum inline-block  font-normal relative text-center
        ...
        {
          subTitle &&
        <p className={`text-sm md:text-base ${modeBlue ? 'text-white' :'text-currentBlack'} font-jost  w-full md:w-[90%] text-center font-normal`}>
          {subTitle}
        </p>
        }
      </div>
