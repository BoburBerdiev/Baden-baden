...
const HomeContent = ({
    ...
    return (
        <>
            ...
            />
            <Header title={langSelect(lang, banner?.title_ru, banner?.title_en, banner?.title_uz)}
                    image={banner?.image}/>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-[20%] pb-[50px] md:pb-20">
                    <div data-aos={'fade-up'}>
                        <AdditionalServicesCard
                            title={langSelect(lang, additionalServices[0]?.title_ru, additionalServices[0]?.title_en, additionalServices[0]?.title_uz)}
                            image={additionalServices[0]?.index_image.image}/>
                    </div>
                    <div data-aos={'fade-up'} data-aos-delay={100} className={' md:mt-[50%]'}>
                        <AdditionalServicesCard
                            title={langSelect(lang, additionalServices[1]?.title_ru, additionalServices[1]?.title_en, additionalServices[1]?.title_uz)}
                            image={additionalServices[1]?.index_image.image}/>
                    </div>
            ...
            <SectionUI
                title={langSelect(lang, aboutTitleAmenities?.title_ru, aboutTitleAmenities?.title_en, aboutTitleAmenities?.title_uz)}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5  ">
                    {
                        amenities?.map((item, id) => (
                            <HotelAmenitiesCard key={item.id} id={id} image={item?.image}
                        ...
                        <div className={'grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-5 md:gap-y-[30px]'}>
                            {
                                landmarkcategories?.map((item, id) => (
                                    <LocationInfos key={item.id} data-aos-delay={id * 100} image={item?.image} title={item?.title}
            ...
            <SectionUI title={langSelect(lang, gallery?.title_ru, gallery?.title_en, gallery?.title_uz)}>
                ...
                <div data-aos={'fade-up'} data-aos-delay={100}
                     className="flex flex-col items-center pb-1.5 pt-5 md:pt-10">
                    ...
                    landmark?.map(item => (
                        <li key={item.id} className='flex justify-between text-base gap-4 md:gap-10 font-thin'>
                            ...
                            <p className="shrink-0 text-[12px] lg:text-base"><span>{item?.distance}</span>
                                <span>{item?.distance_type === 'm' ? t('index.section4.m') : t('index.section4.km')}</span>
                            </p>
                        </li>
                    ))
                }
